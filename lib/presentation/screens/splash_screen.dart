...
import 'onboarding_screen.dart'; // ← CHANGED: was language_screen.dart
...
class _SplashScreenState extends State<SplashScreen>
  ...
  // ── CHANGED: removed LanguageScreen, now uses OnboardingScreen ──
  Future<void> _navigateToNextScreen() async {
    await Future.delayed(const Duration(milliseconds: 2800));
    if (!mounted) return;

    final selectedLanguage = StorageService.instance.getLanguage();

    if (selectedLanguage != null) {
      final isDownloaded =
          await HaramainContentService().isContentDownloaded();
      if (!mounted) return;
      _navigateTo(isDownloaded ? const HomeScreen() : const OnboardingScreen());
    } else {
      // First time — OnboardingScreen has language selection built in
  ...
  Widget _buildBackgroundPattern() {
    return Positioned.fill(child: CustomPaint(painter: _PatternPainter()));
  }
