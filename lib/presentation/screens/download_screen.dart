...
class DownloadScreen extends StatefulWidget {
  ...
  @override
  State<DownloadScreen> createState() =>
      _DownloadScreenState();
}

class _DownloadScreenState
    extends State<DownloadScreen>
  ...
  void _initTasks() {
    _tasks.addAll([
      _TaskItem('Prayer times - Makkah', '🕋'),
      _TaskItem('Prayer times - Madinah', '🕌'),
      _TaskItem('Masjid Al-Haram news', '📰'),
      _TaskItem('Masjid An-Nabawi news', '📰'),
      _TaskItem('Imam schedules', '👨‍💼'),
      _TaskItem('Lesson schedules', '📚'),
      _TaskItem('Friday Khutbah', '🎙️'),
      _TaskItem('Scholars & Sheikhs', '👨‍🏫'),
    ]);
  }

  void _listenToQuranDownload() {
    final downloader = QuranDownloader();
    final current = downloader.getProgress(
      widget.languageCode,
    );
    if (current != null) {
      setState(() => _quranProgress = current);
    }
    ...
    downloader.addProgressListener(
      ...
      (progress) {
        if (mounted) {
          setState(() => _quranProgress = progress);
        }
      },
    );

    downloader.addCompleteListener(
      widget.languageCode,
      (path) {
        if (mounted) {
          setState(() => _quranComplete = true);
        }
      },
    );
  }
  ...
  Future<void> _startDownload() async {
    try {
      final result = await HaramainContentService()
          .downloadAllContent(
  ...
  Widget _buildTitle(BuildContext context) {
    return Column(
      children: [
        ...
        Text(
          _isComplete
              ? 'All content is ready for offline use'
              : _hasError
    ...
    return Container(
      ...
      child: Column(
        children: [
         ...
         if (_quranProgress > 0 && !_quranComplete) ...[
  const SizedBox(height: 10),
  TweenAnimationBuilder<double>(
    tween: Tween<double>(begin: 0, end: _quranProgress),
    ...
    builder: (_, value, __) => ClipRRect(
      ...
      child: LinearProgressIndicator(
        ...
        valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFFD4AF37)),  // ← FIXED
      ),
    ),
  ),
],
...
class _TaskItem {
  ...
  _TaskItem(this.name, this.emoji,
